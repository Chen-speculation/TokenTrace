...
export const translations: Translations = {
    zh: {
        ...
        'Info Highlight': '信息高亮',
        'Paste text to see which words surprise the model most': '粘贴一段文字，看模型认为哪些词最"意外"',
        "- highlight the 'informative' parts": '粘贴任意文本，模型自动标出预测难度大的词汇',
        'Raw Chat': '原始对话',
        'LLM Raw Chat': '原始对话',
        'Chat and inspect every token the model produces': '输入问题，查看模型怎么回答，以及每个词的概率',
        '- chat with explicit raw prompts': '输入 prompt，直接查看模型回复与 token 级信息',
        'Attribution': '上下文归因',
        'Context Attribution': '上下文归因',
        'See which context words drive a specific prediction': '输入一段话和目标词，看哪些词"推动"了这个预测',
        '- attribute a predicted token to its context': '输入上下文与目标词，量化每个来源的贡献份额',
        'Causal Flow': '因果流',
        'LLM Causal Flow': '因果流',
        ...
        'INNERSCOPE - A toolbox for looking inside language models': 'INNERSCOPE 信息透镜 - 探索大模型内部的可视化工具箱',
        ...
        'Raw Chat - Chat and inspect every token the model produces': '原始对话 - 输入问题，查看模型怎么回答，以及每个词的概率',
        'LLM Raw Chat - chat with explicit raw prompts': '原始对话 - 输入 prompt，直接查看模型回复与 token 级信息',
        'Attribution - See which context words drive a specific prediction': '上下文归因 - 输入一段话和目标词，看哪些词"推动"了这个预测',
        'Context Attribution - attribute a predicted token to its context': '上下文归因 - 输入上下文与目标词，量化每个来源的贡献份额',
        ...
        'Both': '两者',
        'Method': '方法',
        'Supports target': '支撑目标',
        'Suppresses target': '抑制目标',
        'Neutral': '中性',
        'Spearman': '斯皮尔曼',
        'Occlusion-based causal attribution. Positive = supports target; negative = suppresses target.':
            '基于遮挡的因果归因。正值 = 支撑目标；负值 = 抑制目标。',
        'Logit Lens': 'Logit Lens',
        'Show per-layer prediction trajectory (Logit Lens)': '显示逐层预测轨迹（Logit Lens）',
        'Projection readouts (final norm + lm_head applied to intermediate layers), not the model\'s literal per-layer computation.':
            '投影读数（final norm + lm_head 套到中间层），不等于模型逐层真实计算。',
        'DAG': 'DAG',
        'Branch Tree': '分叉树',
        'Mode': '模式',
        'Branch tree max nodes reached': '分叉树已达最大节点数',
        'Branch tree max depth reached': '分叉树已达最大深度',
        ...
        'Famous author': '知名作家',
        'Next word': '下一个词',
        'Sentence completion': '句子续写',
        'Story opening': '故事开头',
        'Question': '知识问答',
        'Code': '代码续写',
        '点击折线上的点，查看该层的候选词分布': '点击折线上的点，查看该层的候选词分布',
